...
class UtilTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def _make_model(self, batch_size, num_batches, variable_initializer_value):
    ...
    inputs = tf.compat.v1.data.make_one_shot_iterator(inputs.batch(batch_size)).get_next()
  ...
  def test_map_predictor(self, sub_batch_size):
    input_op = {
        'a': tf.random.normal(shape=(50, 5)),
        'b': tf.random.normal(shape=(50, 5))
    }
    ...
    difference = tf.reduce_mean(
        tf.math.squared_difference(mapped_prediction, unmapped_prediction))
    with tf.compat.v1.Session() as sess:
      self.assertLess(
  ...
  def test_value_op_with_initializer(self):
    ...
    base_value_op = tf.compat.v1.get_variable('value', initializer=0.)
